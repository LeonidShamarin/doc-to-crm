...
class JsonlSink:
    ...
    def write(self, doc: ProcessedDocument) -> bool:
        """
        Повертає True, якщо рядок дописано; False — якщо запис сюди не належить
        або вже був.

        **У леджер потрапляє тільки `auto_ok`** — тобто те, що пройшло всі
        перевірки або було підтверджене людиною. Це не деталь реалізації, а
        визначення: леджер імітує облік, а документ, який чекає на рішення, в
        обліку ще не існує.

        Спокуса писати сюди й `needs_review` (щоб «нічого не загубилось»)
        призводить до тихої втрати даних: рядок ліг би з непідтвердженими
        числами, а після правки рев'ювера дедуплікація за хешем уже не пустила
        б виправлену версію. Черга живе в SQLite, і саме звідти документ
        потрапляє сюди після підтвердження.
        """
